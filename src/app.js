...
io.on('connection', (socket) => {
  ...
  socket.on('join', (data) => {
    ...
    if (
      game == undefined ||
      game.getPlayersArray().some((p) => p == data.rdrCharacter.name) ||
      data.rdrCharacter.name == undefined ||
      game.getPlayersArray().length >= maxPlayers
    ) {
      socket.emit('joinRoom', undefined);
    } else {
      game.addPlayer(data.rdrCharacter, socket);
