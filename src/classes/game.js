...
const Game = function (name, host) {
  ...
  this.assignBlind = () => {
    ...
    for (let i = 0; i < this.players.length; i++) {
      ...
      if (i === this.roundData.bigBlind) {
        this.players[i].setBlind('Ciega grande');
      } else if (i === this.roundData.smallBlind) {
        this.players[i].setBlind('Ciega pequeña');
      } else {
    ...
    this.players[goFirstIndex].setStatus('TURNO');
  };
  ...
  this.updateStage = () => {
    for (let i = 0; i < this.players.length; i++) {
      if (
        ...
        this.players[i].setStatus('TURNO');
  ...
  this.moveOntoNextPlayer = () => {
    ...
    if (this.isStageComplete()) {
      ...
      if (!handOver && numNonFolds == 1) {
        ...
        this.players[currTurnIndex].setStatus('TURNO');
      }
    }
    if (!handOver) {
  ...
  this.addPlayer = (p, socket) => {
    const player = new Player(p, socket, this.debug);
  ...
  this.disconnectPlayer = (player) => {
    ...
    this.emitPlayers('playerDisconnected', { player: player.getUsername(), money: player.getMoney() });
  ...
  this.checkBigBlindWent = (socket) => {
    if (
      this.findPlayer(socket.id).blindValue == 'Ciega grande' &&
  ...
  this.fold = (socket) => {
    ...
    this.foldPot = Number(this.foldPot) + Number(preFoldBetAmount);
    if (
  ...
  this.getPossibleMoves = (socket) => {
    ...
    if (topBet != 0) {
      ...
      if (
        player.blindValue == 'Ciega grande' &&
        !this.bigBlindWent &&
